...
export class AccountLockService implements IAccountLockService {
  ...
  constructor(
    ...
    this.maxAttempts = this.configService.get<number>('security.attemptLockout.maxAttempts', 5);
    this.lockDurationMinutes = this.parseLockDuration(
      this.configService.get<string>('security.attemptLockout.lockDuration', '15m')
    );
  }
  ...
  async incrementFailedAttempts(userId: string): Promise<void> {
    ...
    this.logger.debug(`Failed login attempts for ${userId}: ${updatedAttempts}/${this.maxAttempts}`);
  }
